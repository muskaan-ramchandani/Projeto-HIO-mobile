...
try {
    ...
    if ($idConteudoPertencente > 0 && !empty($email)) {
        // Consulta para buscar os textos relacionados ao e-mail e ao idConteudoPertencente
        $sql = "SELECT t.id, t.titulo, t.texto, p.email 
                FROM Texto t 
                INNER JOIN Professor p ON t.profQuePostou = p.email 
                WHERE t.idConteudoPertencente = :idConteudo AND p.email = :email";
        ...
        if (!empty($textos)) {
            ...
            echo '<p>Você ainda não cadastrou textos para esse conteúdo.</p>';
        }
    } else {
